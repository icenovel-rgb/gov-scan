...
`자격요건.md`를 읽어 회사 프로파일을 구조화한다(아래 키들을 텍스트에서 추출):
지역(시·도) · 업종(KSIC/업태/종목) · 설립일·업력 · 연매출·상시고용 · 기업유형(소상공인/중소/중견/벤처/이노비즈/사회적기업 등) ·
대표자(연령·성별) · 보유 인증 · 관심분야. 이 값들이 다음 단계 판정의 기준이 된다.
...
각 공고에 대해 1단계 프로파일과 공고의 신청자격(대상·지역·업력·업종 제한 등)을 대조한다.
공고 원문 요건은 `scan.json`의 `requirementText`(목록 API가 제공하는 요약) + 필요 시 `detailUrl`을 WebFetch로
보강. 판정은 LLM(너) 자신이 수행하며 결과를 두 값으로 남긴다:

- `eligibility`: **해당** | **해당없음** | **검토필요**(정보 부족)
- `eligibilityReason`: 한 줄 근거(예: "소재지 서울 한정 → 본사 부산이라 해당없음").

보수적으로: 명백히 배제되면 `해당없음`, 명백히 충족되면 `해당`, 애매하면 `검토필요`(스킵하지 말고 DB엔 기록).
